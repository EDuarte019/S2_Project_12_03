"use strict";

/*
   New Perspectives on HTML5 and CSS3, 7th Edition
   Tutorial 10
   Case Problem 3

   Author: Evelyn Duarte 
   Date:  3/5/19 
   
   Filename: ah_report.js
   
   Functions:
   
   calcSum(donorAmt)
      A callback function that adds the current donation amount in the array to the donationTotal variable
   
   findMajorDonors(donorAmt)
      A callback function that returns the value true only if the current donation amount in the array
      is greater than or equal to 1000
      
   donorSortDescending(a, b)
      A callback function used to sort the donation amounts from the array in descending order
      
   writeDonorRow(value)
      A callback function that writes the HTML code for each table row that provides the contact
      information for the donor
      
*/
// The forEach method was given the function of calcSum.
var donationTotal = 0;
donors.forEach(calcSum);
// The variable summaryTable has HTML code placed within which concatinates the variables of donors.length which is the length of the array. donationTotal has a toLocaleString with no parameters.
var summaryTable =
      "<table> <tr><th>Donors</th><td>" + donors.length + "</td></tr><tr><th>Total Donations</th><td>$" +
      donationTotal.toLocaleString() + "</td></tr></table>";

// The document with with id donationSummary gets its HTML code that is within and is placed in the summaryTable variable.
document.getElementById('donationSummary').innerHTML = summaryTable;

// The variable is equal to the filter method that has a function of findMajorDonors.
var majorDonors = donors.filter(findMajorDonors);
// The majorDonors has a filter of sort with a function of donorSortDescending.
majorDonors.sort(donorSortDescending);

// The varible is simply given HTML code placed in a string.
var donorTable = "<table><caption>Major Donors</caption><tr><th>Donation</th><th>Donor ID</th><th>Date</th><th>Name</th><th>Address</th><th>Phone</th><th>E-mail</th></tr>";

// The majorDonors has a method of forEach with a function of writeDonorRow
majorDonors.forEach(writeDonorRow);
// The entire table is not being closed.
donorTable += "</table>";
// The innerHTML with an ID of donorTable is set to the variable of donorTable.
document.getElementById('donorTable').innerHTML = donorTable;

function calcSum(donorAmt) {
      donationTotal += donorAmt[9];
}

function findMajorDonors(donorAmt) {
      return donorAmt[9] >= 1000;
}

function donorSortDescending(a, b) {
      return b[9] - a[9];
}

function writeDonorRow(value) {
      donorTable += "<tr>";
      donorTable += "<td>$" + value[9].toLocaleString() + "</td>";
      donorTable += "<td>" + value[0] + "</td>";
      donorTable += "<td>" + value[10] + "</td>";
      donorTable += "<td>" + value[2] + ", " + value[1] + "</td>";
      donorTable += "<td>" + value[3] + "<br />" + value[4] + ", " + value[5] + " " + value[6] + "</td>";
      donorTable += "<td>" + value[7] + "</td>";
      donorTable += "<td>" + value[8] + "</td>";
      donorTable += "</tr>";
}